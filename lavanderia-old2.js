...
function agendarTimeoutsSeNecessario(sock) {
  ...
  if (agora.isSameOrAfter(fim)) {
    console.log(
      "ℹ️ Lavagem já passou do horário final (ao restaurar). Limpando estado."
    );
  ...
  const avisoEmMs = fim
    .clone()
    .subtract(avisoMinutosAntes, "minutes")
    .diff(agora);
  ...
  if (avisoEmMs > 0) {
    avisoTimeout = setTimeout(async () => {
      try {
        ...
        await sock.sendMessage(
          atual.jid?.includes("@")
            ? atual.jid.replace(/@s.whatsapp.net$/, "") + "@s.whatsapp.net"
            : atual.jid,
          {
    ...
    console.log(
      "ℹ️ Ponto do aviso já passou; não será reenviado ao restaurar."
    );
  }
  ...
  fimTimeout = setTimeout(async () => {
    try {
      ...
      console.error(
        "⚠️ Erro ao processar fim de lavagem agendado:",
        err.message
      );
    }
  }, fimEmMs);
  ...
  console.log(
    `⏱️ Encerramento agendado em ${Math.round(
      fimEmMs / 1000
    )}s (total ${totalMinutos} minutos)`
  );
}
...
6️⃣ Evite misturar roupas delicadas com pesadas.`;
    ...
    if (texto === "3" || texto.includes("iniciar")) {
      ...
      fimTimeout = setTimeout(async () => {
        ...
        try {
          ...
          console.error(
            "⚠️ Erro ao enviar mensagem de fim automático:",
            err.message
          );
        }
    ...
    if (
      ...
      try {
        if (!HGBR_API_KEY)
          throw new Error("Chave HGBR_API_KEY não configurada.");
        const { data } = await axios.get(
          `https://api.hgbrasil.com/weather?key=${HGBR_API_KEY}&city_name=Viamão,RS`
        );
        ...
        if (condicao.includes("chuva") || condicao.includes("tempestade"))
          dica =
            "🌧️ Vai chover! Evite estender roupas ao ar livre e use o varal interno.";
        else if (condicao.includes("nublado"))
          dica =
            "⛅ Dia nublado. Pode lavar, mas prefira secar em local coberto.";
...
📍 *Atualizado automaticamente via HGBrasil API*`;
    ...
    try {
      ...
      console.error(
        "⚠️ Não foi possível enviar mensagem de erro ao grupo:",
        e.message
      );
    }
  }
