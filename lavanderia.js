...
*Respeite o tempo do próximo morador e preserve o equipamento!* 🤝`;
    ...
    // Calcula o tempo de espera (2 horas - 10 minutos = 110 minutos)
    const tempoDeEsperaMs = 110 * 60 * 1000; 

    setTimeout(async () => {
        // Busca quem está na fila (Case 5) no momento do alerta
        const resFila = await axios.post(URL_GOOGLE_SCRIPT, { action: "verificarFila" });
        const proximoDaFila = resFila.data.proximo; // Ajuste conforme o retorno do seu Script
...
_Você receberá um aviso 10 minutos antes do término!_ 🤝`;
    ...
    // Registra a finalização na planilha via Google Script
    ...
    // Dica de encerramento para preservar o equipamento
    ...
    // 1. Verifica se o usuário já está na fila para evitar duplicidade
    ...
    // 2. Registra a entrada na planilha via Google Script
    const resFila = await axios.post(URL_GOOGLE_SCRIPT, { 
        action: "entrarFila", 
        usuario: remetente 
    });

    // 3. Calcula a posição (considerando que quem acabou de entrar é o último)
    ...
    // 4. Informação contextual sobre o tempo
    if (posicao === 1) {
    ...
    // 1. Verifica se a pessoa realmente está na fila antes de tentar remover
    ...
    // 2. Registra a saída na planilha (o Script deve remover pelo ID ou WhatsApp)
    await axios.post(URL_GOOGLE_SCRIPT, { 
        action: "sairFila", 
        usuario: remetente 
    });

    // 3. Lógica de aviso para o próximo (Herança de Posição)
    // Ordenamos a fila por horário de chegada (timestamp) para garantir a justiça
    const filaOrdenada = filaEspera.sort((a, b) => new Date(a.chegada) - new Date(b.chegada));
    
    // Verifica se quem saiu era o primeiro da fila
    const eraOPrimeiro = filaOrdenada[0].usuario === remetente;
    ...
    const mensagemLavanderia = `🧺 *GUIA DE USO CONSCIENTE - LAVANDERIA JK*
...
*Dica:* Deixe sempre o espaço de **um palmo** livre no topo do cesto para as roupas lavarem direito! 🤝`;
    ...
    return sock.sendMessage(grupoId, { text: mensagemLavanderia });
...
_A gestão JK Universitário agradece a compreensão e o zelo com o patrimônio comum!_ 🤝`;
    ...
    // Lógica de Emojis e Recomendações Dinâmicas
    let iconeClima = "🌤️";
    let dicaSecagem = "";
    const condicao = w.condition_slug; // Slug da API HG Brasil (rain, clear_day, etc)
    const vento = parseInt(w.wind_speedy); // Velocidade do vento
...
_Consulte o clima antes de iniciar um ciclo longo!_ 🤝`;
    ...
    return sock.sendMessage(grupoId, { text: mensagemClima });
            ...
            case "10": // Lixo
                const hoje = moment().tz(TIMEZONE).day(); // 2=Ter, 4=Qui, 6=Sab
